...
def valid_kwarg_or_error(func, kwarg_name, pack_name):
    if kwarg_name not in inspect.signature(func).parameters:
        parser.error("Invalid argument {} for {}".format(
            kwarg_name, pack_name))


def build_pack(args: argparse.Namespace) -> TileScopePack:
    if args.strategy_pack not in BASE_PACK:
        parser.error("Invalid strategy pack. Use 'tilescope list' to see"
                     " available pack")
...
parser_tree.add_argument('-l', '--length', type=int,
                         help='Change the length parameter of the pack.')
parser_tree.add_argument('-f', '--fusion', action='store_true',
                         help='Adds fusion to the pack.')
parser_tree.add_argument('-d', '--database', action='store_true',
                         help='Adds database verification to the pack.')
parser_tree.add_argument('-s', '--symmetries', action='store_true',
                         help='Adds symmetries to the pack')
parser_tree.add_argument('-e', '--elementary', action='store_true',
                         help='Makes the pack elementary.')
